 ...
 *
 *
 * @param {caf.rule} rule A simple rule.
 ...
 *
 * @return {caf.ruleEngine} A trie based data structure to speed up checks.
 *
 * @memberof! module:caf_security/rules
 * @alias newRuleEngine
 */
exports.newRuleEngine = function(meOwner, meLocalName, rules) {
    ...
    var addOneRule = function(rule) {
        ...
        checkSimpleRule(rule);
        var p = [
            (rule.ac.caOwner === SELF ? meOwner : rule.ac.caOwner) || WILDCARD,
            (rule.ac.caLocalName === CA_LOCAL ? meLocalName :
             rule.ac.caLocalName) || WILDCARD, rule.methods || WILDCARD
        ];
 ...
 * @param {caf.ruleEngine} rE A trie based data structure to speed up checks.
 *
