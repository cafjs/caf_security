...
var MIN_DURATION_MSEC = 1; // Safety margin before token expires
...
exports.newInstance = function($, spec, cb) {
    ...
    try {
        ...
        var serviceUrl = spec.env.serviceUrl;
        assert.equal(typeof spec.env.serviceUrl, 'string',
                     "'spec.env.serviceUrl' is not a string url");
        ...
        var unrestricted = spec.env.unrestricted;
        assert.equal(typeof spec.env.unrestricted, 'boolean',
                     "'spec.env.unrestricted' is not a boolean");

        var privateKey = loadKey(spec.env.privateKeyFile);    // optional
        var publicKey =  loadKey(spec.env.publicKeyFile);    // optional

        var appPublisher = spec.env.appPublisher;
        assert.equal(typeof appPublisher, 'string',
                     "'spec.env.appPublisher' is not a string");

        var appLocalName = spec.env.appLocalName;
        ...
        var verifyConstraints = function(from, token) {
            try {
                var ca = tokens.splitName(from);
                var target = tokens.newPayload(appPublisher, appLocalName,
                                               ca[0], ca[1], MIN_DURATION_MSEC);
         ...
         * @param {caf.cb} cb0 A callback with an
         *
         ...
         *
         * Only an authenticated owner should be allowed to create new CAs.
         *
        ...
        that.__ca_blockCreate__ = function(from, to) {
            if ((from !== to) || (from === json_rpc.DEFAULT_FROM)) {
                // only an authenticated owner can create a CA.
