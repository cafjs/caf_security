 ...
 * and numbers, ASCII only), and it has at least two characters.
 *
 ...
 */
exports.validExtendedNobody = function(username) {
    var ca = json_rpc.splitName(username);
    return (ca.length === 2) &&
        (ca[0] === json_rpc.DEFAULT_FROM_USERNAME) &&
        (ca[1].length === 2) && // avoid denial of service
        (validUsername(ca[1]));
};
...
exports.validate = function(tokenStr, pubKey) {
    var token = /** @type tokenType*/ (jwt.verify(tokenStr, pubKey));
    if (!checkToken(token)) {
