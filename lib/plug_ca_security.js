...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        spec.env.defaultRules.forEach(function(x) {
                                          acl[rules.computeRuleId(x)] = x;
                                      });
        var caNameSplit = secUtils.splitName($.ca.__ca_getName__());

        var updateRules = function() {
            var values = [];
            Object.keys(acl).forEach(function(x) { values.push(acl[x]);});
            rE = rules.newRuleEngine(caNameSplit[0], caNameSplit[1], values);
        };
       ...
       // transactional ops
        var target = {
            addRuleImpl: function(newRule, cb0) {
                acl[rules.computeRuleId(newRule)] = newRule;
                updateRules();
                cb0(null);
            },
        ...
        that.addRule = function(newRule) {
            ...
            return rules.computeRuleId(newRule);
        };
        ...
        var isAuthorized = function(from, method) {
            caller = null;
            ...
            var fromArray = secUtils.splitName(from);
